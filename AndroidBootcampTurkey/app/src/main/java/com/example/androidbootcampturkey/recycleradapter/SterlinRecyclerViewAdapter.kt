...
class SterlinRecyclerViewAdapter(
    private var faturaList: List<FaturaData>,
    private var moneyList: List<Model>
) :
    RecyclerView.Adapter<SterlinRecyclerViewAdapter.RecyclerViewHolder>() {
        ...
        if (!moneyList.isNullOrEmpty()) {
            ...
            when (currentItem.para_birimi) {
                ...
                "dolar" -> {
                    ...
                    val sonuc =
                        para * (money) * (1.0F / money2)
                    currentItem.para_miktari = sonuc.toString()

                }
                "euro" -> {
                    val money = moneyList.first().GBP.toFloat()
                    val money2 = moneyList.first().EUR.toFloat()
                    val sonuc =
                        para * (money) * (1.0F / money2)
                    currentItem.para_miktari = sonuc.toString()
                }
            ...
            Glide.with(holder.itemView).load(R.drawable.sterlin).fitCenter()
            ...
            holder.view.button2.setOnClickListener { v ->
                ...
                if (positionId >= 0) {
                    try {
                        ...
                        intent.putExtra("para_birimi", "sterlin")
                        intent.putExtra(
                            "fatura_tipi",
