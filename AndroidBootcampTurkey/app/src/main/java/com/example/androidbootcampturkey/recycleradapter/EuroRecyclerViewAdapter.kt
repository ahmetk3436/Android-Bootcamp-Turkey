...
class EuroRecyclerViewAdapter(
    private var faturaList: List<FaturaData>,
    private var moneyList: List<Model>
) :
    RecyclerView.Adapter<EuroRecyclerViewAdapter.RecyclerViewHolder>() {
    ...
    override fun onBindViewHolder(holder: RecyclerViewHolder, position: Int) {
        ...
        if (!moneyList.isNullOrEmpty()) {
            ...
            when (currentItem.para_birimi) {
                ...
                "sterlin" -> {
                    val money = moneyList.first().EUR.toFloat()
                    val money2 = moneyList.first().GBP.toFloat()
                    val sonuc =
                        para * (money) * (1.0F / money2)
                    currentItem.para_miktari = sonuc.toString()
                }
            ...
            Glide.with(holder.itemView).load(R.drawable.euro).fitCenter()
            ...
            holder.view.button2.setOnClickListener { v ->
                ...
                if (positionId >= 0) {
                    try {
                        ...
                        intent.putExtra("para_birimi", "euro")
                        intent.putExtra(
                            "fatura_tipi",
